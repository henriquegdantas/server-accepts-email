...
const debug = require('debug')('server-accepts-email:index') as (
  s: string,
) => void;
...
async function getMailServers(hostname: string): Promise<string[]> {
  ...
  debug(
    `Got ${mxRecords.length} record${
      mxRecords.length === 1 ? '' : 's'
    } for "${hostname}"`,
  );
...
async function testServer(
  client: Client,
  email: string,
  { senderAddress, handleGraylisting }: TestServerOptions,
): Promise<boolean> {
  ...
  if (result.kind === 'greylist') {
    ...
    return pSleep(result.timeout * 1000).then(() => {
      return testServer(client, email, {
        senderAddress,
        handleGraylisting: false,
      });
    });
  }
...
export = async function serverAcceptsEmail(
  email: string,
  options: {
    senderDomain?: string;
    senderAddress?: string;
  ...
  for (const server of servers) {
    try {
      return await globalManager.withClient(
        server,
        senderDomain,
        smtpPort,
        client => {
