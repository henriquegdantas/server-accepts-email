...
export default class Client {
  ...
  constructor(server: string, senderDomain: string, smtpPort: number) {
    this.debug = createDebug(`server-accepts-email:client:${server}`);
    this.pool = new ResourcePool(
      new Factory(server, senderDomain, smtpPort),
      5,
    );
  }
  ...
  test(email: string, { senderAddress }: TestOptions): Promise<TestResult> {
    ...
    return this.pool.use(
      async (connection): Promise<TestResult> => {
        this.debug(`Aquired connection from pool`);

        await connection
          .execute(`MAIL FROM: <${senderAddress}>`)
          .then(response => {
            if (response.code !== 250)
              throw new Error(
                `Server did not accept sender address: ${senderAddress}`,
              );
          });
        ...
        if (
          response.code === 554 &&
          response.comment.includes('this address does not exist')
        ) {
        ...
        if (
          response.code === 501 &&
          response.comment.includes('Bad recipient address syntax')
        ) {
          this.debug(
            'The mailbox name is not allowed (probably ProtonMail or ESMTP Postfix)',
          );
          return { kind: 'answer', answer: false };
        }
        ...
        if (
          response.code === 504 &&
          response.comment.includes('Recipient address rejected')
        ) {
        ...
        if (
          response.code === 501 &&
          response.comment.includes(`<${email}>: `)
        ) {
        ...
        if (
          response.code === 450 &&
          response.comment.includes('unknown user')
        ) {
        ...
        if (
          response.code === 554 &&
          response.comment.includes('Invalid-Recipient')
        ) {
          this.debug('The mailbox is unavailable (probably ESMTP Postfix)');
          return { kind: 'answer', answer: false };
        }
        ...
        if (
          response.code === 451 &&
          response.comment.includes(
            'Greylisting enabled, try again in 1 minutes',
          )
        ) {
          this.debug(
            'Server is applying greylisting, estimated wait time: 60s',
          );
        ...
        if (
          response.code === 451 &&
          response.comment.includes(
            'is not yet authorized to deliver mail from',
          )
        ) {
          this.debug(
            'Server is applying greylisting, estimated wait time: 10m',
          );
          return { kind: 'greylist', timeout: 600 };
        }

        this.debug(
          `Unexpected response: ${response.code} - ${response.comment}`,
        );
