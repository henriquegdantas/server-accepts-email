...
export default class Factory implements ResourcePool.Factory<Socket> {
  ...
  async create() {
    ...
    try {
      const connection = await Socket.connect(this.server, this.smtpPort);

      try {
        const response = await connection.execute(`HELO ${this.senderDomain}`);
        if (response.code !== 250)
          throw new Error(
            `Server did not accept sender domain: ${this.senderDomain}`,
          );
  ...
  async destroy(connection: Socket, error: Error | null) {
    ...
    try {
      ...
      if (response.code === 421) {
        this.debug(
          'Server sent 421 in response to QUIT, ignoring (probably ProtonMail)',
        );
        ...
        throw new Error(
          `Unexpected response code to QUIT command: ${response.code}`,
        );
      }
    } finally {
      globalLimit.release();
