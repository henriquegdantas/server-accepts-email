...
export default class Socket {
  ...
  static async connect(server: string, smtpPort: number) {
    const self = new Socket(server, smtpPort);
  ...
  private constructor(server: string, smtpPort: number) {
    this.debug = createDebug(`server-accepts-email:socket:${server}`);

    this.debug(`Connecting to "${server}"`);
    this.socket = net.connect(smtpPort || 25, server);
